...
@section('buttons')
    <a href="{{ route('admin.cms.create') }}" class="btn btn-sm btn-primary mt-4">Add CMS Page</a>
@endsection
                ...
                <div class="row">
                    <div class="col-12">
                        <table class="table table-bordered ma7zouz-tables">
                            ...
                            <tbody>
                            @foreach($cms_contents as $content)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $content->title }}</td>
                                    <td>
                                        <label class="switch">
                                            <input {{ $content->status ? 'checked' : '' }} type="checkbox" class="cms_status" data-id="{{ $content->id }}">
                                            <span class="slider round"></span>
                                        </label>
                                    </td>
                                    <td>
                                        <a class="text-success" href="{{ route('admin.cms.edit', ['id' => $content->id]) }}"><i class="bx bx-pen"></i></a>
                                        <a class="text-danger" href="javascript:void(0);" onclick="deleteCMS('{{ $content->id }}')"><i class="bx bx-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
...
@section('script')
    <script>
        ...
        function deleteCMS(id) {
            let url = '{{ route('admin.cms.delete', ['id' => ':id']) }}';
