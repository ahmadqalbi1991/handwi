...
@section('buttons')
    <button class="btn btn-success btn-sm mt-4" id="cms_save">Save CMS</button>
...
@section('content')
    ...
    <div class="custom">
        <div class="row">
            <div class="col-12">
                <form action="{{ route('admin.cms.save') }}" id="cms-form" >
                    @if(!empty($content))
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" value="{{ !empty($content) ? $content->title : '' }}" name="title" id="title" required class="form-control">
                                    </div>
                                </div>
    ...
    <script>
        ...
        $('#cms_save').on('click', function () {
            if ($('#cms-form').valid()) {
                ...
                $.ajax({
                    url: '{{ route("admin.cms.save") }}', // Form action URL
                    type: 'POST', // HTTP method
                    data: {
                        title: $('#title').val(),
                        description: quill.root.innerHTML,
                        status: $('#status').val(),
                        id: "{{ !empty($content) ? $content->id : '' }}"
